...
class Generator:
    ...
    def generate_datasets(self, door_path, handle_path, door_data, handle_data, mesh):
        ...
        if door_name == "9961408" or door_name == "9961410" or door_name == "9961405" or door_name == "9961401":
            door_height_scaler = 0.7
        elif door_name == "9961409" or door_name == "9961406" or door_name == "9961404" or door_name == "9961402":
            door_height_scaler = 0.7
        elif door_name == "9961407" or door_name == "":
            door_height_scaler = 0.5
        elif door_name == "" or door_name == "":
        ...
        if name == "9960005" or name == "" or name == "":
            handle_length_scaler = 1.4
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961501" or name == "9961503":
        ...
        with open(urdf_save_path, 'w') as f:
            ...
            if name == "9961408":
                joint_bis_2 = str(- board_min_points[0] * door_height_scaler) + " " + str(-board_max_points[2] * door_height_scaler) + " " + str(-board_min_points[1] * door_height_scaler - 0.01)
            else:
                joint_bis_2 = str(- board_min_points[0] * door_height_scaler) + " " + str(-board_max_points[2] * door_height_scaler) + " " + str(-board_min_points[1] * door_height_scaler)    
            f.write('\t\t<origin xyz="{}"/>\n'.format(joint_bis_2))
...
if __name__ == "__main__":
    # 参数
    parser = argparse.ArgumentParser(description='compose objects from fridge and handle')
    parser.add_argument('--fridge_dae_path', type=str, default='./assets/3dware_fridge', help='Path to the fridge dae file')
    parser.add_argument('--handle_path', type=str, default='./assets/fridge_handle', help='Path to the fridge handle dae file')
    parser.add_argument('--save_path', type=str, default='./generated_datasets/fridge_datasets/', help='Path to the urdf file')
    parser.add_argument('--handle_length', type=float, default=0.15, help='Path to the urdf file')
    parser.add_argument('--fridge_list', nargs='+', required=True, help="fridge body parts list")
    parser.add_argument('--fridge_handle_list', nargs='+', required=True, help="fridge handle parts list")
    args = parser.parse_args()

    generator = Generator(args)
    
    for fridge in args.fridge_list:
        # 拿到bounding box
        fridge_path = os.path.join(args.fridge_dae_path, fridge)
        mesh = trimesh.load(os.path.join(fridge_path, "texture_dae", "board.dae"))
        with open(os.path.join(fridge_path, 'bounding_box.json'), 'r') as f:
            fridge_bounding_data = json.load(f)

        for handle in args.fridge_handle_list:
            ...
            generator.generate_datasets(fridge_path, handle_path, fridge_bounding_data, handle_bounding_data, mesh)