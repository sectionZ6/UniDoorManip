...
class Generator:
    ...
    def generate_datasets(self, door_path, handle_path, door_data, handle_data, mesh):
        ...
        door_height_scaler = 0.6
        if door_name == "9961111" or door_name == "9961112" or name == "" or name == "":
            door_height_scaler = 0.2
        elif door_name == "9961132" or door_name == "":
            door_height_scaler = 1.4
        elif door_name == "9961141" or door_name == "9961128" or door_name=="9961180":
            door_height_scaler = 1
        elif door_name == "9961127" or door_name == "":
            door_height_scaler = 0.25
        door_bounding_box = {}
        ...
        if name == "9961204" or name == "9961206" or name == "9961210" or name == "9961222" or name == "":
            handle_length_scaler = 0.5
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961214" or name == "9961207" or name=="":
            handle_length_scaler = 0.7
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name=="9961213":
            handle_length_scaler = 0.6
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961238":
            handle_length_scaler = 0.25
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961218" or name == "9961228" or name == "":
            handle_length_scaler = 0.3
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961224" or name == "":
            handle_length_scaler = 0.27
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961217" or name == "9961229":
            handle_length_scaler = 0.1
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961230" or name == "":
            handle_length_scaler = 0.9
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        elif name == "9961234" or name == "":
            handle_length_scaler = 0.008
            print("name-{}-sacle-{}".format(name, handle_length_scaler))
        handle_bounding_box = {}
        ...
        safe_scale = str(door_height_scaler) + " " + str(door_height_scaler) + " " + str(door_height_scaler)
        ...
        with open(urdf_save_path, 'w') as f:
            ...
            f.write('<robot name="right-pull-door">\n')
            ...
            f.write('\t\t\t<geometry>\n')
            f.write('\t\t\t\t<mesh filename="texture_dae/frame.dae" scale="{}"/>\n'.format(safe_scale))
            f.write('\t\t\t</geometry>\n')
            f.write('\t\t</visual>\n')
            ...
            link_bis = str(-board_min_points[0]*door_height_scaler) + ' 0 0'
            joint_bis = str(door_height_scaler*board_min_points[0]) + ' 0 0'
            ...
            f.write('\t\t\t<origin xyz="{}"/>\n'.format(link_bis))
            f.write('\t\t\t<geometry>\n')
            f.write('\t\t\t\t<mesh filename="texture_dae/board.dae" scale="{}"/>\n'.format(safe_scale))
            f.write('\t\t\t</geometry>\n')
            f.write('\t\t</visual>\n')
            ...
            joint_bis_2 = str(-board_min_points[0]*door_height_scaler) + " 0 " + str(board_max_points[1] * door_height_scaler + 0.008)
            f.write('\t\t<origin xyz="{}"/>\n'.format(joint_bis_2))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--safe_dae_path', type=str, default='./assets/3dware_safe', help='Path to the safe dae file')
    parser.add_argument('--handle_path', type=str, default='./assets/safe_handle', help='Path to the safe handle dae file')
    parser.add_argument('--save_path', type=str, default='./generated_datasets/safe_datasets/', help='Path to the urdf file')
    parser.add_argument('--lever_handle_length', type=float, default=0.18, help='Path to the urdf file')
    parser.add_argument('--safe_list', nargs='+', required=True, help="safe body parts list")
    parser.add_argument('--safe_handle_list', nargs='+', required=True, help="safe handle parts list")
    
    args = parser.parse_args()

    generator = Generator(args)
    
    for safe in args.safe_list:
        # 拿到bounding box
        safe_path = os.path.join(args.safe_dae_path, safe)
        mesh = trimesh.load(os.path.join(safe_path, "texture_dae", "board.dae"))
        with open(os.path.join(safe_path, 'bounding_box.json'), 'r') as f:
            safe_bounding_data = json.load(f)

        for handle in args.safe_handle_list:
            ...
            generator.generate_datasets(safe_path, handle_path, safe_bounding_data, handle_bounding_data, mesh)