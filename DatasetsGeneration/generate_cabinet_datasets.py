...
class Generator:
    ...
    def generate_datasets(self, door_path, handle_path, door_data, handle_data, mesh):
        ...
        if door_name == "9961307" or door_name == "" or name == "" or name == "":
            door_height_scaler = 0.7
        elif door_name == "9961320" or door_name == "":
            door_height_scaler = 0.7
        elif door_name == "9961340" or door_name == "":
            door_height_scaler = 1.2
        door_bounding_box = {}
        ...
        handle_length_random = np.random.uniform(self.args.handle_length - 0.002, self.args.handle_length + 0.002)
        ...
        with open(urdf_save_path, 'w') as f:
            ...
            if name + "-lock-right.dae" in os.listdir(handle_path):
                ...
                joint_bis_2 = str(-board_min_points[0]*door_height_scaler) + " " + str(-(board_max_points[2]*door_height_scaler)) + " " + str(board_max_points[1]*door_height_scaler)
                f.write('\t\t<origin xyz="{}"/>\n'.format(joint_bis_2))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cabinet_dae_path', type=str, default='./assets/3dware_cabinet', help='Path to the cabinet dae file')
    parser.add_argument('--handle_path', type=str, default='./assets/round_door_handle', help='Path to the handle dae file')
    parser.add_argument('--save_path', type=str, default='./generated_datasets/cabinet_datasets/', help='Path to the urdf file')
    parser.add_argument('--handle_length', type=float, default=0.066, help='Path to the urdf file')
    parser.add_argument('--cabinet_list', nargs='+', required=True, help="cabinet body parts list")
    parser.add_argument('--round_handle_list', nargs='+', required=True, help="cabinet handle parts list")
    args = parser.parse_args()
    
    generator = Generator(args)
    
    for door in args.cabinet_list:
        # 拿到bounding box
        door_path = os.path.join(args.cabinet_dae_path, door)
        ...
        for handle in args.round_handle_list:
