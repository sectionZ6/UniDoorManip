...
class Generator:
    ...
    def generate_datasets(self, door_path, handle_path, door_data, handle_data, mesh):
        ...
        door_height_random = np.random.uniform(self.args.door_height - 0.1, self.args.door_height + 0.1)
        door_height_scaler = door_height_random / (-door_data["min"][2] + door_data["max"][2])
        door_bounding_box = {}
        ...
        handle_length_random = np.random.uniform(self.args.handle_length - 0.004, self.args.handle_length + 0.004)
        
        handle_length_scaler = handle_length_random / (-handle_data["handle"][0][0] + handle_data["handle"][1][0])
        handle_bounding_box = {}
        ...
        door_scale = str(door_height_scaler) + " " + str(door_height_scaler) + " " + str(door_height_scaler)
        ...
        with open(urdf_save_path, 'w') as f:
            ...
            if name + "-lock-right.dae" in os.listdir(handle_path):
                f.write('\t\t<visual name="handle">\n')
                f.write('\t\t\t<origin xyz="0 0 0"/>\n')
                f.write('\t\t\t<geometry>\n')
                f.write('\t\t\t\t<mesh filename="texture_dae/{}-handle-right.dae" scale="{}"/>\n'.format(name, handle_scale))
                f.write('\t\t\t</geometry>\n')
                f.write('\t\t</visual>\n')
                ...
                f.write('\t\t\t<origin xyz="0 0 0"/>\n')
                f.write('\t\t\t<geometry>\n')
                f.write('\t\t\t\t<mesh filename="texture_dae/{}-lock-right.dae" scale="{}"/>\n'.format(name, handle_scale))
                f.write('\t\t\t</geometry>\n')
                f.write('\t\t</visual>\n')
                ...
                joint_bis_2 = str(board_max_points[0]*door_height_scaler - 0.08 - board_min_points[0]*door_height_scaler) + " " + str(-(board_max_points[2]*door_height_scaler - board_min_points[2]*door_height_scaler - 0.8)) + " " + str(board_max_points[1]*door_height_scaler)
                f.write('\t\t<origin xyz="{}"/>\n'.format(joint_bis_2))
...
if __name__ == "__main__":
    # 参数
    parser = argparse.ArgumentParser(description='compose objects from door and handle')
    parser.add_argument('--door_dae_path', type=str, default='./assets/3dware_door', help='Path to the door dae file')
    parser.add_argument('--handle_path', type=str, default='./assets/round_door_handle', help='Path to the handle dae file')
    parser.add_argument('--save_path', type=str, default='./generated_datasets/round_door_datasets', help='Path to the urdf file')
    parser.add_argument('--handle_length', type=float, default=0.064, help='Path to the urdf file')
    parser.add_argument('--door_height', type=float, default=1.8, help='Path to the urdf file')
    parser.add_argument('--round_door_list', nargs='+', required=True, help="round_door body parts list")
    parser.add_argument('--round_door_handle_list', nargs='+', required=True, help="round_door handle parts list")

    args = parser.parse_args()

    generator = Generator(args)

    for door in args.round_door_list:
        # 拿到bounding box
        door_path = os.path.join(args.door_dae_path, door)
        mesh = trimesh.load(os.path.join(door_path, "texture_dae", "board.dae"))
        with open(os.path.join(door_path, 'bounding_box.json'), 'r') as f:
            door_bounding_data = json.load(f)
        
        for handle in args.round_door_handle_list:
            ...
            generator.generate_datasets(door_path, handle_path, door_bounding_data, handle_bounding_data, mesh)