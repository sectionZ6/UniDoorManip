...
def train(conf):
    ...
    data_features = ['pc', 'contact_point','finger_pos', 'contact_rot', 'affordance_score']

    # load network model
    model_def = utils.get_model_module(conf.model_version)

    # create models
    network = model_def.ScoreNetwork(pc_input_feat=conf.pc_input_feat, pc_feat_dim=conf.pc_feat_dim,
                                      action_feat_dim=conf.action_feat_dim, action_dim=conf.action_dim)
    ...
    train_dataset.load_affordance_data(conf.affordance_data_path)
    ...
    val_dataset.load_affordance_data(conf.affordance_data_path)
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        for train_batch_ind, batch in train_batches:
            ...
            losses = score_forward(batch=batch, data_features=data_features, network=network, device=conf.device)
            ...
            while val_fraction_done <= train_fraction_done and val_batch_ind + 1 < val_num_batch:
                ...
                with torch.no_grad():
                    ...
                    losses = score_forward(batch=batch, data_features=data_features, network=network,
                                            device=conf.device)
...
def score_forward(batch, data_features, network, device):
    # prepare input
    pc = batch[data_features.index('pc')].to(device)  # B x 4096 x 3
    # handle_pc = pc[pc[:, :, 3] == 3].contiguous().reshape(-1, 200, 4)

    contact_point = batch[data_features.index('finger_pos')].to(device)
    contact_rot = batch[data_features.index('contact_rot')].to(device)
    action_9d = torch.cat([contact_point, contact_rot[:, :2, :].reshape(-1, 6)], dim=-1)
    gt_score = batch[data_features.index('affordance_score')].to(device)
    losses = network.get_loss(pc[:, :, :3], action_9d, gt_score.squeeze(-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_suffix', type=str, default='lever_rule_0630', help='exp suffix')
    parser.add_argument('--train_mode', action='store_true', default=False)
    parser.add_argument('--model_version', type=str, default='model_score', help='model def file')
    ...
    parser.add_argument('--affordance_data_path', type=str,
                        default='../../logs', help='affordance data path')
    ...
    parser.add_argument('--action_feat_dim', type=int, default=128, help='feature dim of critic network')
