...
class FrankaSliderWindow(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, is_multi_agent=False, log_dir=None):
        ...
        if self.figure:
            pass
            # self.gym.set_light_parameters(self.sim, 0, gymapi.Vec3(0.85, 0.85, 0.85), gymapi.Vec3(0.85, 0.85, 0.85), gymapi.Vec3(10, 10, 10))
            # self.gym.set_light_parameters(self.sim, 1, gymapi.Vec3(0,0,0), gymapi.Vec3(0,0,0), gymapi.Vec3(2, 2, 2))
    ...
    def create_camera(self, env_ptr, env_id):
        if self.figure:
            ...
            # 左侧camera
            self.gym.set_camera_location(camera_handle_right, env_ptr, gymapi.Vec3(1.5, 1.2, 1.8), gymapi.Vec3(0.01, 0.0, 1.2))
            # 右侧camera
    ...
    def get_adjust_hand_pose(self):
        ...
        if self.gapartnet_baseline:
            ...
            self.goal_pos_offset_tensor[:, 2] = range_z/3
            self.goal_pos_offset_tensor[:, 0] = 0
            self.goal_pos_offset_tensor[:, 1] = range_y/2
    ...
    def _obj_init_pose(self, min_dict, max_dict, name):
        ...
        height = 0.1
        if name == "9001" or name == "9004":
            height = 0.6
        elif name == "9008":
            height = 0.2
        elif name == "9010":
            height = 0.3
        elif name == "9012" or name == "9018":
            height = 0.4
