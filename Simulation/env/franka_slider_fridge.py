...
class FrankaSliderFridge(BaseEnv):
    ...
    def _load_obj(self, env_ptr, env_id):
        ...
        if self.task == "leverdoor":
            ...
            # set handle props, randomlization
            # random handle dof [0.3, 0.6]*dof
            # random_upper = ((1 / 12) * np.random.rand() + 0.5) * door_dof_props['upper'][1]
            door_dof_props['upper'][1] = math.pi / 4
            # door_dof_props['upper'][0] = 0.0

            # random physics props [0.8, 1.0]
            
            # random_phy = (0.3*np.random.rand()+0.5)
            # door_dof_props['stiffness'][1] = 5.0 * random_phy
            # door_dof_props['friction'][1] = 1.5 * random_phy
    ...
    def _obj_init_pose(self, min_dict, max_dict, name):
        ...
        height = 0.1
        if name == "1409" or name == "1407":
            height = 0.3
        elif name == "1408":
            height = 0.4
        elif name == "1402" or name == "1401":
            height = 0.2
