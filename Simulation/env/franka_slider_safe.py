...
class FrankaSliderSafe(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, is_multi_agent=False, log_dir=None):
        ...
        # 加载资源列表
        # for name in cfg["env"]["asset"]["trainAssets"][self.load_block]:
        #     ipdb.set_trace()
        #     self.door_train_name_list.append(cfg["env"]["asset"]["trainAssets"][self.load_block][name]["name"])
        # for name in cfg["env"]["asset"]["testAssets"]:
        #     self.door_val_name_list.append(cfg["env"]["asset"]["testAssets"][self.load_block][name]["name"])
        ...
        self.row_length = 0.08
        self.col_length = 0.08
        self.x_init = 0.77
        self.y_init = 0
    ...
    def create_camera(self, env_ptr, env_id):
        if self.figure:
            ...
            # 左侧camera
            self.gym.set_camera_location(camera_handle_right, env_ptr, gymapi.Vec3(1.0, 1.0, 1.4), gymapi.Vec3(0.01, 0.0, 0.8))
            # handle camera
            self.gym.set_camera_location(camera_handle_left, env_ptr, gymapi.Vec3(1.5, 1.2, 1.8), gymapi.Vec3(0.01, 0.0, 1.2))
    ...
    def get_adjust_hand_pose(self):
        ...
        if self.gapartnet_baseline:
            ...
            self.goal_pos_offset_tensor[:, 2] = range_z/4
    ...
    def _obj_init_pose(self, min_dict, max_dict, name):
        ...
        height = 0.6
        if name == "1122" or name == "" or name == "" or name == "1116":
            height = 0.4
        elif name == "1121" or name == "1137" or name == "1128":
            height = 0.4
        elif name == "1164" or name == "1172" or name == "":
            height = 0.55
        elif name == "1167" or name == "1161" or name == "1154" or name == "":
            height = 0.65
        elif name == "1179" or name == "xx":
            height = 0.45
        elif name == "1141" or name == "":
            height = 0.7
    ...
    def step(self, actions):
        ...
        if self.collectData:
            # if (self.progress_buf[0] == 1) | (self.progress_buf[0] == 51):
            #     print("-----------------------1")
            #     self.collect_data()
    ...
    def _perform_actions(self, actions):
        ...
        # ipdb.set_trace()
        self.pos_act[:,-4:-2] = torch.where((dist<0.108), close, open)

        self.open_door_stage = ((self.door_handle_dof_tensor[:, 0] >= 0.5 * (self.door_actor_dof_upper_limits_tensor[:, 1] - self.door_actor_dof_lower_limits_tensor[:, 1])) |
